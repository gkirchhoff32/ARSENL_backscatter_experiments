...
def data_organize(dt, data_dir, fname, window_bnd, max_lsr_num, exclude_shots=True, repeat_num=1):
    ...
    valid_flight_idx = np.where((flight_time >= window_bnd[0]) & (flight_time < window_bnd[1]))
    flight_time = flight_time[valid_flight_idx]
    ttag_sync_idx = ds.time_tag_sync_index.values
    ttag_sync_idx = ttag_sync_idx[valid_flight_idx]
